/**
 * Hardcoded per-token pricing for known models.
 *
 * Anthropic token semantics:
 *   promptTokens      = non-cached input only
...
export function getPricing(model: string): ModelPricing | null {
  ...
  // Prefix match — handles versioned names like claude-haiku-4-5-20251001
...
export function calculateCost(model: string, tokens: TokenCounts): number | null {
  ...
  if (pricing.isAnthropic) {
    // promptTokens is already the non-cached portion
    return (
    ...
    // promptTokens includes cached; subtract to get the non-cached portion

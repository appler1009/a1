...
describe('Monthly spend limit', () => {
  it('is undefined by default on new users', async () => {
    const user = await db.createUser('limit-default@example.com');
    expect(user.monthlySpendLimitUsd).toBeUndefined();
    const fetched = await db.getUser(user.id);
    expect(fetched?.monthlySpendLimitUsd).toBeUndefined();
  });

  it('can be set and retrieved via updateUser', async () => {
    const user = await db.createUser('limit-set@example.com');
    const updated = await db.updateUser(user.id, { monthlySpendLimitUsd: 5.0 });
    expect(updated?.monthlySpendLimitUsd).toBe(5.0);

    const fetched = await db.getUser(user.id);
    expect(fetched?.monthlySpendLimitUsd).toBe(5.0);
  });

  it('can be cleared back to undefined', async () => {
    const user = await db.createUser('limit-clear@example.com');
    await db.updateUser(user.id, { monthlySpendLimitUsd: 2.5 });
    const cleared = await db.updateUser(user.id, { monthlySpendLimitUsd: undefined });
    // Passing undefined leaves the existing value unchanged (no-op update)
    expect(cleared?.monthlySpendLimitUsd).toBe(2.5);
  });

  it('is included in getAllUsers results', async () => {
    const user = await db.createUser('limit-all@example.com');
    await db.updateUser(user.id, { monthlySpendLimitUsd: 3.0 });
    const all = await db.getAllUsers();
    const found = all.find(u => u.id === user.id);
    expect(found?.monthlySpendLimitUsd).toBe(3.0);
  });

  it('survives fractional values accurately', async () => {
    const user = await db.createUser('limit-fractional@example.com');
    await db.updateUser(user.id, { monthlySpendLimitUsd: 0.50 });
    const fetched = await db.getUser(user.id);
    expect(fetched?.monthlySpendLimitUsd).toBe(0.50);
  });
});
